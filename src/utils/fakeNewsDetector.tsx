...
// --- BEGIN: Server integration shim ---
// Attempts to call a server endpoint to perform AI analysis.
// If the server call fails, falls back to the original client-side heuristic.
export async function analyzeWithServer(text: string): Promise<DetectionResult | null> {
  try {
    ...
    if (!res.ok) {
      console.warn('Server analysis returned non-OK status', res.status);
      return null;
    }
    ...
    if (data && data.classification && typeof data.confidence === 'number') {
      return {
        ...
        explanation: data.explanation || { reasoning: data.explanation || '', keywords: data.keywords || [], sources: data.sources || [] },
    ...
    console.warn('Server analysis failed:', e);
    return null;
  }
}
// --- END server integration shim ---
